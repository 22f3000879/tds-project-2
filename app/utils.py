...
def try_decode_base64(text: str):
    # Decodes the hidden instructions in Step 2
...
def fetch_external_resources(base_url, html_content):
    """
    Downloads ALL linked assets: JS, JSON, CSV, Images, Audio, ZIPs.
    """
    assets = ""
    # Regex to find all relevant file types
    patterns = [
        r'src=["\']([^"\']+\.(?:js|png|jpg|jpeg|opus|mp3|wav|zip|json))["\']',
    ...
    for relative_url in urls:
        ...
        try:
            ...
            # 1. BINARY (Images, ZIPs) -> Base64 for Python analysis
            if filename.endswith(('.png', '.jpg', '.jpeg', '.zip')):
                b64 = base64.b64encode(resp.content).decode('utf-8')
                assets += f"\n--- BINARY FILE: {filename} ---\nBASE64:{b64}\n"
            
            # 2. AUDIO -> Transcribe immediately
            ...
            # 4. TEXT DATA (JS, JSON, CSV, MD, YAML)
                ...
                # If JS, fetch its imports (nested logic)
...
def parse_file_content(file_url: str):
    # This is a fallback for direct file links
    try:
